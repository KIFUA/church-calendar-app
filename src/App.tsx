                  ...
                  <div className={`flex-grow ${showPreacherTable ? 'pl-0.5 pr-1 py-1 space-y-0.5' : 'pl-1 pr-2 py-2 space-y-1'} min-h-[40px] min-w-0`}>
                    {dayEvents.length > 0 ? dayEvents.map((ev, i) => {
                      ...
                      return (
                          <div 
                            key={i} 
                            className={`grid ${showPreacherTable ? 'grid-cols-[auto_6.5fr_125px]' : 'grid-cols-[auto_7.5fr_105px] md:grid-cols-[auto_8.5fr_90px] lg:grid-cols-[auto_9.5fr_70px]'} items-stretch ${showPreacherTable ? 'gap-0.5' : 'gap-0.5 md:gap-2 lg:gap-0.5'} ${showPreacherTable ? 'py-px px-1 pl-1' : 'py-0.5 md:py-1 lg:py-0 pl-1 md:pl-1.5 lg:pl-0.5 pr-0.5 md:pr-1 lg:pr-0.5'} ${showPreacherTable ? 'rounded-lg' : 'rounded-xl md:rounded-2xl lg:rounded-lg'} border border-slate-200 shadow-sm hover:border-blue-300 hover:shadow-md transition-all relative group/event overflow-hidden ${isCleaning ? 'bg-slate-200' : 'bg-white'}`}
                          >
                          ...
                          <div className={`${isCleaning ? 'col-span-2' : 'col-span-1'} flex flex-col gap-0.5 md:gap-1 border ${showPreacherTable ? 'rounded-md' : 'rounded-lg md:rounded-xl'} ${showPreacherTable ? 'px-1 py-0.5' : 'px-1.5 md:px-2 py-1 md:py-1.5'} min-w-0 ${ev.align === 'center' ? 'text-center items-center' : ev.align === 'right' ? 'text-right items-end' : 'text-left items-start'}`} style={{ borderColor: darkenHex(WEEKDAY_COLORS[d.weekdayIndex], 0.15) }}>
                            <div 
                              className={`${showPreacherTable ? 'text-[6px]' : 'text-[clamp(7px,1.5vw,14px)] lg:text-[16px]'} leading-tight tracking-tight group-hover/event:scale-[1.01] transition-transform w-full whitespace-pre-wrap break-words min-w-0 ${ev.isBold !== false ? 'font-black' : 'font-medium'} ${ev.isItalic === true ? 'italic' : ''} ${ev.isUnderline === true ? 'underline' : ''} ${ev.isUppercase !== false ? 'uppercase' : ''}`}
